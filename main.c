...
int validate_mobile(char *mob)
{
    ...
    for(j=0; mob[j]!='\0'; j++)
        if(!isdigit(mob[j]))
            return 0;
    if(j!=11 || mob[0]!='0' || mob[1]!='1') // number must consist of 11 digits and first two digits are 0 and 1
        return 0;
    return 1;
}
typedef struct
...
int save()
{
    ...
    else if(x==2)
    {
        ...
        printf("invalid entry, choose only either 1 or 0\n\n");
    return 0;
}
void Add()
{
    ...
    for(i=0; i<count; i++)
    {
        if((strcmp(A[count].acc_num, A[i].acc_num))==0)
        {
    ...
    {
        printf("your mobile number must consist of 11 digits and the first 2 digits must be 0 and 1\n\n");
        return;
    }
...
void Withdraw()
{
    ...
    if (found_acc(accountNumber, &i))
    {
        ...
        if(validate_balance(amount))
        {
            ...
            else printf("Invalid balance\n");

        }
        else
            printf("Invalid withdrawal amount or exceeding maximum limit.\n");
    }
    else
    {
        ...
        if (strcmp(sourceAccount, destinationAccount) != 0)
        {
            if (found_acc(destinationAccount, &j))
            {
                ...
                if (validate_balance(amount))
                {
                    ...
                    {
                        printf("Invalid transfer amount or exceeding maximum limit.\n");
                    }
                }
                else
                {
...
void Delete()
{
    ...
    for (int i = 0; i < count; i++)
    {
        if (strcmp(A[i].acc_num, accountNumber) == 0)
        {
            // Swap the account to be deleted with the last account in the array
            A[i] = A[count - 1];
            ...
            if(save())
            {
                ...
                return;
            }// Exit the function after the operation
        }
    }
